...
typedef struct
{
	...
	int out;			//出库数目
	int amount;			//最终库存
	int warning_value;	//警戒值（缺货的标准）
	int state;			//库存状态(是否低于警戒值)
}goods;
goods s[M];				//存放 货品 信息 
goods r[M];				//存放 入 库货品信息 
goods t[M];				//存放 出 库货品信息 
...
void Display()		//显示 库存情况
{
	...
	for(i=0,j=1;i<P;i++,j++)
	{
		...
		else if(s[i].state==0)	//库存值 不小于 警戒值白字显示
		{
			HANDLE handle = GetStdHandle(STD_OUTPUT_HANDLE);
			SetConsoleTextAttribute(handle, FOREGROUND_INTENSITY | FOREGROUND_RED| FOREGROUND_GREEN | FOREGROUND_BLUE); 
			printf("%-9d  %-10s%-10d%-10d%-10d%-10d%-10d\n",s[i].num,s[i].name,s[i].stock,s[i].in,s[i].out,s[i].amount,s[i].warning_value);
